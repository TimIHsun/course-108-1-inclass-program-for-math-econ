...
def chunk_lang(chunk_start):
    if chunk_start.find("```{r") != -1:
        return('r') #要用return不能用print,否則1.3language會有儲存的問題
    elif chunk_start.find("```{python") != -1:
        return('python')
chunk_lang
```
...
```{python ans12}
lineIndex = [filelines[i].find("```") for i in range(len(filelines))]
lineIndexlist = [i for i in enumerate(lineIndex)]
index_3ticks = [lineIndexlist[i][0] for i in range(len(lineIndexlist)) if lineIndexlist[i][1]==0]
index_3ticks
```
...
```{python ans13}
lanlist = [ filelines[index_3ticks[i]] for i in range(len(index_3ticks)) if int(i/2) == i/2 ]

dict_chunks = {
  "start":[index_3ticks[i]+1 for i in range(len(index_3ticks)) if int(i/2) == i/2 ],
  "end":[index_3ticks[i]-1 for i in range(len(index_3ticks)) if int(i/2) != i/2],
  "language":[chunk_lang(lanlist[i]) for i in range(len(lanlist))]
 }
dict_chunks
...
for i in range(len(dict_chunks["start"])):
      m=dict_chunks["start"][i]
      while m<dict_chunks["end"][i]+1:
        python_code_list.append(filelines[m]) #一個一個加
        m+=1
      
python_code_list
```
#要用while否則儲存格式會有問題
comprehention 會產生多個list 在答案list裡
```{python}
las = []
for i in range(0,3):
   las.append(filelines[i:i+2])
lasL = [las[i:i+2] for i in range(len(las))] #一群加一群加一群
```
...
找出danceInfo裡uid為'5d35ec95d083a398fcdc5e14'的資料並在其'showInfo'添加如下的dictionary形式資訊：  
```
...
```{python ans23}
danceInfo[allUIDs.index('5d35ec95d083a398fcdc5e14')]["showInfo"].append({'time': '2019/10/30 15:00:00', 'location': '新北市三峽區大學路151號', 'locationName': '國立臺北大學'})
...
Mtest=list([])
for i in range(1,3):
      Mtest.append([M[i][j] for j in range(2,5)])

print(Mtest)
```

```{python ans32}
M_partial=[M[1:3][0][2:5],M[1:3][1][2:5]]
M_partial
```
