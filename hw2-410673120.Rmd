...
```{python ans14}
python_code_list=[
filelines[dict_chunks["start"][i]:dict_chunks["end"][i]+1] for i in range(len(dict_chunks["start"]))
]
# python_code_list
```
```{python}
...
for i in range(len(dict_chunks["start"]):
  if dict_chunks["language"] == "python":
    lang.append(filelines[dict_chunks["start"][i]:dict_chunks["end"][i]+1])
