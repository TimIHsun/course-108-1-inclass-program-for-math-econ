...
```{python ans21}
num_taoyuan = sum(
  [dict_business["公司所在地"][i][0:3] == "桃園市" for i in range(0,len(dict_business["公司所在地"]))]
  )
...
```{python ans22}
num_largeCapital = sum(
  [dict_business["資本額"][i] > 500000 for i in range(0,len(dict_business["資本額"]))]
) 
...
danceInfoStr = [
  "【"+danceInfo[i]["title"]+"】" +    
  "將於"+str(danceInfo[i]["showInfo"][0]["time"])+
  "在"+danceInfo[i]["showInfo"][0]["location"]+
  "演出" 
  for i in range(0,numDance)
  ]
danceInfoStr
