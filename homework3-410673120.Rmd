...
df_jointP.set_axis(hw3data['mood index'], axis=1)
df_jointP.set_axis(hw3data['weather index'], axis=0)
...
```{python ans13}
P_mood_cond_weather=df_jointP.copy()
for i in range(len(marginP_weather)):
  P_mood_cond_weather.iloc[i]=df_jointP.iloc[i].divide(marginP_weather[i])

P_weather_cond_mood=df_jointP.copy()
for i in range(len(marginP_mood)):
  P_weather_cond_mood.iloc[:,i]=df_jointP.iloc[:,i].divide(marginP_mood[i])
...
```{python ans15}
moodLogi = max(P_mood_cond_weather.loc["cloudy",:]) == P_mood_cond_weather.loc["cloudy",:]
est_mood= P_mood_cond_weather.columns[moodLogi][0]
...
```{python ans21}
x_partition=np.linspace(2,3,100)
...
def x_star(x_partition, alpha=0.5):
    ...
    for i in range(len(x_partition)-1):
        m.append(x_partition[i]+alpha*(x_partition[i+1]-x_partition[i]))
    return(m)
...
logSum=0
for i in range(len(x_partition)-1):
  logSum+=f(x_star(x_partition))[i]*(x_partition[i+1]-x_partition[i])
rsum = logSum
q2ans['ans23']=rsum
```
...
def riemannsum(fun,interval,N=101,alpha=0.5):
    x_partition=np.linspace(interval[0],interval[1],N)
    logSum=0
    for i in range(len(x_partition)-1):
      logSum+=fun(x_star(x_partition,alpha))[i]*(x_partition[i+1]-x_partition[i])
    return(logSum)
